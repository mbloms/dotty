...
class RingInt extends Ring[Int] where
  val zero = 0
  val one  = 1
  val add  = (x, y) => x + y
  val sub  = (x, y) => x - y
...
class RingIntExpr(using Quotes) extends Ring[Expr[Int]] where
  val zero = '{0}
  val one  = '{1}
  val add  = (x, y) => '{$x + $y}
  val sub  = (x, y) => '{$x - $y}
...
class RingComplex[U](u: Ring[U]) extends Ring[Complex[U]] where
...
class RingPV[U: ToExpr](u: Ring[U], eu: Ring[Expr[U]])(using Quotes) extends Ring[PV[U]] where
...
trait VecOps[Idx, T] where
  val reduce: ((T, T) => T, T, Vec[Idx, T]) => T

class StaticVecOps[T] extends VecOps[Int, T] where
...
class ExprVecOps[T: Type](using Quotes) extends VecOps[Expr[Int], Expr[T]] where
...
class Blas1[Idx, T](r: Ring[T], ops: VecOps[Idx, T]) where
