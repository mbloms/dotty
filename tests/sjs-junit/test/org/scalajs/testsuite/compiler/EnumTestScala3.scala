...
object EnumTestScala3 where

  enum Color1 derives CanEqual where
    case Red, Green, Blue

  enum Color2 extends java.lang.Enum[Color2] derives CanEqual where
    case Red, Green, Blue

  // test "non-simple" cases with anonymous subclasses
  enum Currency1(val dollarValue: Double) derives CanEqual where
    case Dollar    extends Currency1(1.0)
    case SwissFanc extends Currency1(1.09)
    case Euro      extends Currency1(1.18)

  enum Currency2(val dollarValue: Double) extends java.lang.Enum[Currency2] derives CanEqual where
